...
# Freeze the hidden inputs of the decoder layers, which do not
#  tap into the encoder.
for layer in dec_rnn_layers:
    GRULayer_freeze(layer, v_gen_input)

# Readout the bidi-context from the encoder.
...
# A generator procedure, which automatically selects the best replies
#  (lowest perplexity).
...
def reset(bot, update, args):
    if update.message.chat_id != 191745228:
        update.message.reply_text("you are not the Creator.")
        return
